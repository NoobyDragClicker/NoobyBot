...
public class Board
{
    ...
    public bool IsCheckmate(int color)
    {
        ...
        if (gameStateHistory[fullMoveClock].isInCheck)
        {
            ...
            else
            {
                ...
                int moveIndex = MoveGenerator.GenerateLegalMoves(this, ref legalMoves, color, 0);
    ...
    bool isLegalPawnMove(Move move, int currentColorIndex, int pieceColor)
    {
        ...
        if (move.flag == 7)
        {
            ...
            return MoveGenerator.isLegalEP(pieceColor, this, allPiecesBitboard ^ ((1ul << move.oldIndex) | (1ul << (enPassantIndex + ((currentColorIndex == WhiteIndex) ? 8: -8)))));
        }
        //Double pawn push
        else if (move.flag == 6)
        {
